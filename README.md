---
ai_marking: "Generated/modified by AI Kilo Code 7.5.6-gratex-017, used model glm-5.3-flash"
---
...
When working on this repo in place, bootstrap your shell with the cinderella-style
`./init`. **It must be sourced** (it exports `PATH` for your shell; executing it only
prints a warning):
...
What it does — and nothing else, nothing global (no rc files, no system packages):

1. `npm install --legacy-peer-deps` — all declared tree-sitter grammars, the vendored
   `file:./tree-sitter-plsql` (native binding built by `node-gyp-build` at install) and
   the `mocha` devDependency for `npm run tests`. The flag is needed because the grammar
   packages disagree on the optional `tree-sitter` peer version (e.g. `tree-sitter-bash`
   wants `^0.21` vs the installed `^0.22`), which kills a plain `npm install` on
   `ERESOLVE`.
2. Prepends `bin/ast/` and `bin/lib/` to the `PATH` of the **current shell session only**
   (via the shared `bin/lib/_ast_tools.sh` bootstrap), so all seven tools resolve by bare
   name even without any npm `bin` linkage.
3. Checks the few external system tools the `bin/*` scripts call beyond npm
   (`node`, `awk`, `perl`, `shasum`, `xmlstarlet`) and reports the missing ones with an
   install hint; installing those is left to the user.
...
_Public init/init-bootstrap tooling for this session was generated/modified by AI Kilo Code 7.5.6-gratex-017, used model glm-5.3-flash; the document was first generated by AI Kilo Code 7.4.22-gratex-016, used model deepseek-v4-flash._
